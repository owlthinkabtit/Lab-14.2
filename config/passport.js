  ...
  new GitHubStrategy(
    ...
    async (accessToken, refreshToken, profile, done) => {
      try {
        ...
        console.log(profile)
        console.log((profile.emails && profile.emails[0].value) || `${profile.username}@gmail.com`)
        // If it's a new user, create a record in our database
        const newUser = new User({
          githubId: profile.id,
          username: profile.username,
          email: (profile.emails && profile.emails[0].value) || `${profile.username}@gmail.com` , // Some providers return an array of emails
        });
